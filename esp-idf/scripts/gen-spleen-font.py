...
BSD-2-Clause. This emits the printable-ASCII range (0x20..0x7E) plus whichever
box-drawing glyphs (U+2500 block) the BDF carries, as a fixed 5x8 cell font
matching LVGL's `lv_font_fmt_txt` layout — the same structure LVGL's own
`lv_font_unscii_8.c` uses, so it compiles unchanged here. ASCII goes in one
contiguous cmap; the box-drawing glyphs (sparse — Spleen only ships the "light"
line set) go in a second SPARSE_TINY cmap so absent variants still resolve to
"no glyph" and get filtered out by callers.
...
FIRST, LAST = 0x20, 0x7E          # printable ASCII (contiguous range)
BOX_FIRST, BOX_LAST = 0x2500, 0x257F  # box-drawing block (sparse; only those the BDF has)
CELL_W, CELL_H = 5, 8
    ...
    while i < n:
        if lines[i].startswith("STARTCHAR"):
            ...
            while i < n and not lines[i].startswith("ENDCHAR"):
                i += 1
            i += 1
            if enc is None or bbx is None:
                continue
            if not (FIRST <= enc <= LAST) and not (BOX_FIRST <= enc <= BOX_LAST):
                continue
...
def main():
    ...
    ascii_codes = list(range(FIRST, LAST + 1))                            # contiguous
    box_codes = sorted(cp for cp in glyphs if BOX_FIRST <= cp <= BOX_LAST)  # sparse, present-only
    blank = pack_cell([[0] * CELL_W for _ in range(CELL_H)])              # fallback for absent ASCII

    # Glyph ids run 1.. in emission order: ASCII first (one contiguous cmap),
    # then the box-drawing glyphs (one sparse cmap). id 0 stays reserved.
    entries = []   # (codepoint, packed-bytes, bitmap_index)
    idx = 0
    for cp in ascii_codes:
        cell = glyphs.get(cp)
        data = pack_cell(cell) if cell else blank
    ...
    L.append(" * the U+2500 box-drawing glyphs Spleen ships).")
    ...
    L.append("")
    if box_codes:
        # SPARSE_TINY unicode_list: offsets from BOX_FIRST, ascending (bsearch'd).
        L.append("static const uint16_t box_unicode_list[] = {")
        L.append("    " + ", ".join("0x%04x" % (cp - BOX_FIRST) for cp in box_codes))
        L.append("};")
        L.append("")
    L.append("static const lv_font_fmt_txt_cmap_t cmaps[] = {")
    L.append("    {")
    L.append("        .range_start = %d, .range_length = %d, .glyph_id_start = 1," % (FIRST, len(ascii_codes)))
    L.append("        .unicode_list = NULL, .glyph_id_ofs_list = NULL, .list_length = 0, .type = LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY")
    L.append("    }%s" % ("," if box_codes else ""))
    if box_codes:
        # range_length is the fast-reject bound: a codepoint resolves only if
        # (cp - range_start) < range_length, so it must cover the last offset.
        box_span = (box_codes[-1] - BOX_FIRST) + 1
        L.append("    {")
        L.append("        .range_start = %d, .range_length = %d, .glyph_id_start = %d,"
                 % (BOX_FIRST, box_span, 1 + len(ascii_codes)))
        L.append("        .unicode_list = box_unicode_list, .glyph_id_ofs_list = NULL, .list_length = %d, .type = LV_FONT_FMT_TXT_CMAP_SPARSE_TINY"
                 % len(box_codes))
        L.append("    }")
    L.append("};")
    L.append("")
    ...
    L.append("    .cmap_num = %d," % (2 if box_codes else 1))
    L.append("    .bpp = 1,")
    ...
    with open(out, "w") as f:
        f.write("\n".join(L))
    print("wrote %s (%d ascii + %d box glyphs, %d bitmap bytes)"
          % (out, len(ascii_codes), len(box_codes), idx))
